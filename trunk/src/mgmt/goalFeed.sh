...
#
# Overview:
#   Accesses the Goal Feed of the Google Analytics Management API through cURL.
#   Users are prompted to input their Account, Web Property and Profile IDs to
...
if [ -z "$account_id" ] || [ "$account_id" == "~all" ]; then
  ...
  if [ -z "$web_property_id" ] || [ "$web_property_id" == "~all" ]; then
    ...
    read -p "Please enter a Profile ID (default will use ~all): " profile_id
...
/analytics/feeds/datasources/ga/accounts/$account_id/webproperties/$web_property_id/profiles/$profile_id/goals\
?prettyprint=true"
